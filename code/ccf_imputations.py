...
def impute_spatial_coordinates(adata_query, adata_ref, k=3, epsilon=1e-10, 
    ...
    if similarity_transform=='softmax':
        weights = imputations.weight_softmax_dst(distances, tau=tau)
    elif similarity_transform=='inverse':
        weights = imputations.weight_inverse_dst(distances, epsilon=epsilon)
    elif similarity_transform=='gaussian':
        weights = imputations.weight_gaussian_dst(distances, sigma=sigma)
    else:
